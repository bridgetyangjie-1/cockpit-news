        ...
        with DDGS() as ddgs:
            for query in queries:
                for r in ddgs.text(query, max_results=max_results // 2, timelimit='w'):
                    ...
                    all_results.append({
                        "title": title, "url": url,
                        "snippet": body[:400], "site_name": site_name
                    })
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .news-link {{
            display: inline-block; font-size: 13px; color: #667eea; text-decoration: none;
            padding: 4px 12px; border-radius: 4px; background: #f0f0ff;
        }}
...
def main():
    ...
    if items:
        ...
        items = [{"title": n["title"], "title_en": n["title"], "url": n["url"], "site_name": n["site_name"], "summary": n["snippet"], "summary_en": n["snippet"]} for n in news[:5]]
    print("📝 生成页面...")
